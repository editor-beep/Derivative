...
function buildBody(insight) {
    ...
    const playerDid = oneClause(template?.playerDid(insight, context) ?? `You connected ${context.leadWord} with ${context.secondWord} through ${context.root}`, "You connected the key words");
    const matters = oneClause(template?.matters(insight, context) ?? `That matters because this gives you a faster read on ${context.language} word history`, "That matters because now the logic is reusable");
