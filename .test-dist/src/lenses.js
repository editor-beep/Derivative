...
function applyLens(insight, lens, r) {
    if (!lens.applicableTo.includes(insight.type)) {
        return { ...insight, lens: exports.LENSES[0] };
    }
    const out = {
        ...insight,
        lens,
    ...
    switch (lens.id) {
        ...
        case "ETYMOLOGY_FILTER": {
            if (insight.type === "ROOT" && insight.data.entryPaths) {
                const paths = insight.data.entryPaths;
        ...
        case "FALSE_TWIN": {
            if (insight.type === "ROOT" && insight.data.impostors?.length) {
                const impostors = insight.data.impostors;
        ...
        case "REGISTER_SORT": {
            if ("registers" in insight.data && insight.data.registers) {
                const regs = insight.data.registers;
