...
exports.buildPlayablePuzzle = buildPlayablePuzzle;
const SORT_TYPES = [
    "SUPPLETIVE",
    "COLLISION",
    "PIE",
...
function shufflePool(pool, seed) {
    return [...pool]
        .map((word, index) => ({ word, score: stableHash(`${seed}:${word}:${index}`) }))
        .sort((a, b) => a.score - b.score)
        .map(({ word }) => word);
}
function deriveSystemGuess(type) {
    switch (type) {
        case "BORROWED":
        case "TOPONYM":
            return "Borrowing";
...
function buildSortPlayablePuzzle(puzzle) {
...
function buildPlayablePuzzle(puzzle) {
